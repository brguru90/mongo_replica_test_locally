...
START_FROM_PORT=27028
END_TO_PORT=27030

for i in $(seq $START_FROM_PORT $END_TO_PORT);
do   
    echo "creating service in port $i"
    # bash ./create_service.sh $i $cur_dir $CUR_HOST $MASTER_HOST $MASTER_USERNAME $MASTER_PASSWORD "arb"
    bash ./create_service.sh $i $cur_dir $CUR_HOST $MASTER_HOST $MASTER_USERNAME $MASTER_PASSWORD "secondary"
done
